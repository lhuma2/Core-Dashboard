...
export function InspectionForm({ clients, sites }: { clients: ClientRow[]; sites: SiteRow[] }) {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-28">
      ...
      <div className="sticky top-14 z-20 -mx-4 px-4 py-3 bg-white/90 backdrop-blur border-b border-gray-200 flex items-center justify-between">
        <div>
      ...
      {/* Client + site */}
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 mt-4 space-y-3">
        <div>
      ...
      <div className="space-y-3 mt-3">
        {areas.map((a, ai) => {
          ...
          return (
            <div key={a.key} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-bold text-gray-900">{a.name}</p>
                {areaRects > 0 && <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-amber-600"><AlertTriangle className="w-3.5 h-3.5" />{areaRects}</span>}
              </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 lg:left-56 z-30 bg-white/95 backdrop-blur border-t border-gray-200 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
          <span className={`text-sm font-bold ${BAND_COLOR[band]}`}>{overall != null ? `${overall}%` : '—'}</span>
