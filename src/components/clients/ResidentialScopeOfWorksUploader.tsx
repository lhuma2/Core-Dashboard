...
import { uploadResidentialScopeImageAction, removeResidentialScopeImageAction } from '@/actions/residentialJobs'
...
export function ResidentialScopeOfWorksUploader({ jobId, imageUrl }: Props) {
  ...
  async function handleSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    fd.append('file', file)
    const result = await uploadResidentialScopeImageAction(jobId, fd)
    setUploading(false)
    if (result.error) {
  ...
  async function handleRemove() {
    setUploading(true)
    await removeResidentialScopeImageAction(jobId)
    setUploading(false)
    router.refresh()
  }
  ...
  return (
    <div className="bg-white border border-gray-200/70 rounded-2xl px-5 py-4">
      <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">Scope of Works Image</p>
      ...
      {imageUrl ? (
        <div className="space-y-2">
          ...
          >
            <Image src={imageUrl} alt="Scope of works" fill className="object-cover" sizes="600px" />
