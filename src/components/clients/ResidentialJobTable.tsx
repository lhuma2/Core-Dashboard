...
export interface ResidentialJobRow {
  id: string
  client_name: string
  address: string
...
export function ResidentialJobTable({ jobs, deleteAction }: ResidentialJobTableProps) {
  ...
  function handleDelete(id: string) {
    if (!confirm('Remove this residential clean? This cannot be undone.')) return
    setDeletingId(id)
  ...
  const columns: Column<ResidentialJobRow>[] = [
    {
      key: 'client_name',
      header: 'Client',
      render: (j) => (
        <Link href={`/clients/residential/${j.id}`} className="block">
    ...
    <>
      ...
      <div className="hidden sm:block">
        <Table
          ...
          columns={columns}
          keyExtractor={(j) => j.id}
          emptyMessage="No residential cleans scheduled yet."
        />
      </div>
      ...
      <div className="sm:hidden divide-y divide-gray-100">
        {jobs.length === 0 ? (
          <p className="text-center text-sm text-gray-400 py-10">No residential cleans scheduled yet.</p>
        ) : (
          jobs.map((j) => (
