...
export async function updateResidentialJobCleanerCostAction(id: string, cost: number | null) {
  const supabase = createClient()
  const db = supabase as any
  const { error } = await db.from('residential_jobs').update({ cleaner_cost: cost }).eq('id', id)
...
export async function deleteResidentialJobAction(id: string) {
  const supabase = createClient()
  const db = supabase as any
  await db.from('residential_jobs').delete().eq('id', id)
  revalidatePath('/clients')
}

// ─── Real-time tracking for residential cleans (mirrors src/actions/bondJobs.ts) ────
...
export async function startResidentialJobAction(jobId: string) {
  const supabase = createClient()
...
export async function cancelStartResidentialJobAction(jobId: string) {
  const supabase = createClient()
  const profile = await getCurrentProfile(supabase)
  if (!profile) return { error: 'Not authenticated' }
